...
| 操作 | 命令行示例 | 说明 |
| :--- | :--- | :--- |
| **容器备份** | `ops backup run vps-1:my-app` | 自动识别并全量打包容器配置与数据 |
| **备份并重命名** | `ops backup run vps-1:web-dev --as web-prod` | 生成的 Compose 与任务直接改名为新名称 |
| **跨机还原与启动** | `ops restore run my-app --target-server vps-2` | 还原并默认自动拉起容器与灌库 |
| **还原时改名** | `ops restore run my-app --target-server vps-2 --as new-app` | 还原到目标 VPS 时指定新项目名启动 |
| **仅恢复文件** | `ops restore run my-app --target-server vps-2 --no-start` | 仅解压数据，不启动容器 |
| **指定快照还原** | `ops restore run my-app --target-server vps-2 --snapshot <id>` | 还原到指定的历史版本快照 |
