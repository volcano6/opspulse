...
当我们在 WSL (`/home/user/`) 备份的配置需要还原到 VPS (`/root/`) 时，路径通常是不匹配的。如果我们直接还原，会导致在 VPS 上创建不期望的 `/root/home/user/` 目录结构。
...
在上述命令中：
- `/home/user/.zshrc.local` 将被准确地还原到 VPS 的 `/root/.zshrc.local`
- 过程完全自动化，无需手动 `mv`。
...
- SSH: `~/.ssh/config`, `~/.ssh/known_hosts` (注意：私钥最好保存在 1Password，而不是备份到 S3)
